...
install_framework()
{
  ...
  normalize_cef_framework_layout "${destination}/$(basename "$1")"
...
{
  local framework="$1"
  if [ "$(basename "$framework")" != "Chromium Embedded Framework.framework" ] || [ -d "$framework/Versions" ]; then
    return
  fi
  ...
  if [ -d "$framework/Resources" ]; then
    mv "$framework/Resources" "$version_dir/Resources"
  fi
  if [ -d "$framework/Libraries" ]; then
    mv "$framework/Libraries" "$version_dir/Libraries"
  fi
  if [ -f "$framework/Chromium Embedded Framework" ]; then
    mv "$framework/Chromium Embedded Framework" "$version_dir/Chromium Embedded Framework"
  fi
...
if [[ "$CONFIGURATION" == "Debug" ]]; then
  ...
  install_framework "${BUILT_PRODUCTS_DIR}/flutter_webrtc/flutter_webrtc.framework"
  ...
  install_framework "${BUILT_PRODUCTS_DIR}/native_context_menu/native_context_menu.framework"
  ...
  install_framework "${BUILT_PRODUCTS_DIR}/video_compress_ds/video_compress_ds.framework"
  install_framework "${BUILT_PRODUCTS_DIR}/video_player_macos/video_player_macos.framework"
  install_framework "${PODS_ROOT}/../Flutter/ephemeral/.symlinks/plugins/webview_cef/macos/third/cef/Chromium Embedded Framework.framework"
  install_framework "${BUILT_PRODUCTS_DIR}/webview_cef/webview_cef.framework"
  install_framework "${BUILT_PRODUCTS_DIR}/window_manager/window_manager.framework"
  install_framework "${PODS_XCFRAMEWORKS_BUILD_DIR}/WebRTC-SDK/WebRTC.framework"
fi
