...
class GameController extends GetxController {
  ...
  void _initializeGame() {
    try {
      ...
      // 初始化位置
      playerPosition.value = const Offset(400, 300);
      joystickPosition.value = const Offset(40, 40);
    ...
    try {
      if (playerScore.value > 0) {
        ...
        if (playerScore.value >
            (highScores.isNotEmpty ? highScores.first['score'] : 0)) {
          _logGameEvent('创造新高分: ${playerScore.value}');
        }

        _logGameEvent(
            '游戏结果保存: 分数${playerScore.value}, 关卡${currentLevel.value}');
      }
    } catch (e) {
  ...
  void movePlayer(Offset direction) {
    ...
    try {
      ...
      _logGameEvent(
          '玩家移动: (${direction.dx.toStringAsFixed(1)}, ${direction.dy.toStringAsFixed(1)})');
    } catch (e) {
  ...
  void handleKeyboardInput(KeyEvent event) {
    if (!isGameRunning.value || isPaused.value || !isKeyboardEnabled.value) {
      return;
    }
  ...
  bool get canMove =>
      isGameRunning.value && !isPaused.value && !isGameOver.value;
  ...
  String get playerStatus =>
      '生命: ${playerHealth.value}, 分数: ${playerScore.value}, 生命数: ${playerLives.value}';
  String get joystickInfo =>
      '摇杆: 角度${joystickAngle.value.toStringAsFixed(1)}°, 距离${joystickDistance.value.toStringAsFixed(1)}';
