...
class CompressController extends GetxController {
  ...
  Future<void> _getImageDimensions(File file) async {
    try {
      ...
      print('Image dimensions: ${originalWidth.value} x ${originalHeight.value}');
    } catch (e) {
  ...
  Future<void> compressImage() async {
    ...
    try {
      ...
      _logAction('压缩图片: 压缩比 ${(compressionRatio.value * 100).toStringAsFixed(1)}%');
  ...
  Future<void> compressWithCustomSettings({
    ...
    try {
      ...
      _logAction('自定义压缩: 质量${(quality.value * 100).toStringAsFixed(0)}%, 尺寸${compressedWidth.value}x${compressedHeight.value}');
  ...
  String get originalFileSize {
    if (originalFile.value != null) {
      try {
        final bytes = originalFile.value!.readAsBytesSync().lengthInBytes;
  ...
  String get compressedFileSize {
    if (compressedFile.value != null) {
      try {
        final bytes = compressedFile.value!.readAsBytesSync().lengthInBytes;
        return '${(bytes / 1024).toStringAsFixed(2)} KB';
      } catch (e) {
        return '未知';
      }
  ...
  String get originalDimensions => '${originalWidth.value} x ${originalHeight.value}';
  String get compressedDimensions => '${compressedWidth.value} x ${compressedHeight.value}';
  String get compressionInfo => '压缩比: ${(compressionRatio.value * 100).toStringAsFixed(1)}%';
